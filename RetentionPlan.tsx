...
interface RetentionPlanPageProps {
  onBack: () => void;
...
const RetentionPlanPage: React.FC<RetentionPlanPageProps> = ({ onBack, editingId }) => {
  const { user } = useAuth();
    ...
    if (editingId) {
        ...
        if (plan) {
            setFormData({
                ...
                tungaloyProductDesc: (plan as any).tungaloyProductDesc || '',
                reasonForTrial: (plan as any).reasonForTrial || '',
                competitorName: (plan as any).competitorName || '',
                competitorProductDesc: (plan as any).competitorProductDesc || '',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-2xl font-bold text-slate-800">{editingId ? 'EDIT RETENTION PLAN' : 'RETENTION PLAN'}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="bg-orange-500 text-white p-3 rounded-t-lg font-bold">Trial Plan & Solution</div>
            <div className="space-y-3">
              ...
              <textarea placeholder="Tungaloy Product Description" className="w-full border rounded p-2 outline-none" rows={2} value={formData.tungaloyProductDesc} onChange={e => setFormData({...formData, tungaloyProductDesc: e.target.value})} />
              <textarea placeholder="Reason for trial plan" className="w-full border rounded p-2 outline-none" rows={2} value={formData.reasonForTrial} onChange={e => setFormData({...formData, reasonForTrial: e.target.value})} />
              <input placeholder="Competitor Name" className="w-full border-b p-2 outline-none" value={formData.competitorName} onChange={e => setFormData({...formData, competitorName: e.target.value})} />
              <input placeholder="Competitor Product Desc" className="w-full border-b p-2 outline-none" value={formData.competitorProductDesc} onChange={e => setFormData({...formData, competitorProductDesc: e.target.value})} />
          ...
          <div className="space-y-4">
             <div className="bg-blue-600 text-white p-3 rounded-t-lg font-bold">Technical Info</div>
             <div className="space-y-3">
                <div className="space-y-3">
               ...
               {[
                 ...
                 { label: 'Expected Value (Lakhs)', key: 'valueLakhs', type: 'number', readOnly: true }
               ].map(field => (
                 <div key={field.key} className="space-y-1">
                    ...
                    <input 
                      ...
                      className={`w-full border-b p-1 outline-none ${field.readOnly ? 'bg-slate-100 text-slate-500' : ''}`}
                      value={(formData as any)[field.key]} 
                      onChange={e => !field.readOnly && setFormData({...formData, [field.key]: field.type === 'number' ? Number(e.target.value) : e.target.value})} 
                    />
                 </div>
               ))}
             </div>
        ...
        <div className="p-6 bg-slate-50 flex justify-end">
          <button className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-12 rounded-xl shadow-lg transition-transform active:scale-95">
             {editingId ? 'Update Plan' : 'Submit Retention Plan'}
          </button>
        </div>
      </form>
...
export default RetentionPlanPage;
