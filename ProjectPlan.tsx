...
import { MACHINE_TYPES, STATUS_OPTIONS } from '../constants';
import { PlanType, UserRole, Attachment, AIParameter } from '../types';

// Declaration to prevent build error if @types/node is missing
...
const ProjectPlanPage: React.FC<ProjectPlanPageProps> = ({ onBack, editingId }) => {
  const { user } = useAuth();
    ...
    if (editingId) {
        ...
        if (plan) {
            setFormData({
                ...
                machineDetail: plan.machineDetail,
                componentMaterial: plan.componentMaterial,
                materialHardness: plan.materialHardness,
                inputCondition: plan.inputCondition,
                cycleTime: (plan as any).cycleTime,
                toolList: (plan as any).toolList,
                requiredDate: (plan as any).requiredDate,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-2xl font-bold text-slate-800">{editingId ? 'EDIT PROJECT PLAN' : 'NEW PROJECT PLAN'}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="bg-orange-500 text-white p-3 rounded-t-lg font-bold flex justify-between items-center">
                <span>Machine Type</span>
                <span className="text-[10px] uppercase bg-white/20 px-2 rounded">Step 1</span>
            </div>
            
            <div className="space-y-3">
                <select className="w-full border rounded-lg p-3 outline-none focus:ring-2 focus:ring-orange-500 font-bold text-slate-700" value={formData.machineType} onChange={e => setFormData({...formData, machineType: e.target.value})}>
            ...
            <div className="space-y-4 mt-6">
              <div className="flex gap-4">
                 <div className="flex-1 space-y-1">
                    <label className="text-sm font-bold text-slate-600">Required Date</label>
                    <input type="date" className="w-full border-b p-2 outline-none focus:border-orange-500" value={formData.requiredDate} onChange={e => setFormData({...formData, requiredDate: e.target.value})} />
                 </div>
              </div>
              ...
              <div className="flex gap-6 pt-4">
                  <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-lg hover:bg-slate-50 w-full">
                      <input type="checkbox" className="w-5 h-5 accent-orange-600" checked={formData.cycleTime} onChange={e => setFormData({...formData, cycleTime: e.target.checked})} />
                      <span className="font-bold text-slate-700">Cycle Time</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-lg hover:bg-slate-50 w-full">
                      <input type="checkbox" className="w-5 h-5 accent-orange-600" checked={formData.toolList} onChange={e => setFormData({...formData, toolList: e.target.checked})} />
                      <span className="font-bold text-slate-700">Tool List</span>
                  </label>
              </div>

              {/* Attachments */}
              <div className="space-y-2 pt-4 border-t">
                  <label className="text-sm font-bold text-slate-600">Attachments (Photo/Doc)</label>
          ...
          <div className="space-y-4">
            <div className="bg-blue-600 text-white p-3 rounded-t-lg font-bold">Component Details</div>
            <div className="grid grid-cols-1 gap-4">
               ...
               {[
                 ...
                 { label: 'Value (Lakhs)', key: 'valueLakhs', type: 'number' }
               ].map(field => (
                 <div key={field.key} className="flex justify-between items-center border-b pb-1">
                    <label className="text-sm font-medium text-slate-600">{field.label}</label>
                    <input 
                      type={field.type || 'text'} 
                      className="text-right border-none outline-none font-semibold text-slate-800 focus:text-orange-600" 
        ...
        <div className="p-6 bg-slate-50 flex justify-end">
          <button className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-12 rounded-xl shadow-lg transition-transform active:scale-95">
             {editingId ? 'Update Plan' : 'Submit New Project'}
          </button>
        </div>
      </form>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
                ...
                <div className="p-6 overflow-y-auto bg-slate-50/50 flex-1 space-y-4">
                    ...
                    <button 
                        ...
                        className="w-full bg-slate-800 text-white px-4 py-3 rounded-lg font-bold text-sm hover:bg-slate-900 disabled:opacity-50"
                    >
